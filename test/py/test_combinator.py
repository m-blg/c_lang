from lib.parsing.c.lexer import parse_string, LexerState
...
def test_choice():
    assert isinstance(
        sequence([choice([parse_string("a"), parse_string("ab")]), parse_string("c")])(
            LexerState("abc")
        ),
        Err,
    )
    ...
    tok, rest = sequence(
        [choice([parse_string("ab"), parse_string("a")]), parse_string("c")]
    )(LexerState("abc")).unwrap()
    match tok:
        case ["ab", "c"]:
            pass
        case _:
