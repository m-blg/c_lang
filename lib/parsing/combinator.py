...
class ParserError[S](Protocol):
    def push(self, state: S, expected, found):
        ...
...
def range[
    T, S, E
](parser: Parser[T, S, E], min: int, max: int) -> Parser[list[T], S, E]:
    # assert min <= max
...
def filter[
    T, S, E
](parser: Parser[T, S, E], pred: Callable[[T], bool]) -> Parser[T, S, E]:
...
def dispatch[
    ...
    look_ahead_count: int, parsers: list[tuple[Callable[[list], bool], Parser[T, S, E]]]
) -> Parser[T, S, E]:
    def out_parser(state: S) -> ParserRet[T, S, E]:
...
def default_init(cls):
    args_s = ", ".join(
        map(lambda p: f"{p[0]}: {p[1].__name__}", cls.__annotations__.items())
    )
...
def result_runner[
    T, R, E: ParserError
](f: Callable[..., Generator[Result[T, E], T, R]]):
    @wraps(f)
