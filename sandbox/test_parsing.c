...
//     // gen_dbg_fmt_fmt()
//     println_pref(gen_dbg_fmt, &struct1);
...
//     ASSERT_OK(c_parse_t_keyword_kind(&pstate, C_KEYWORD_STRUCT, &tok));
//     dbgp(c_token, tok, .data = &text);
//     ASSERT_OK(c_parse_ident(&pstate, &ident));
//     c_ast_unparse_println(ident, ident, nullptr);
//     ASSERT_OK(c_parse_t_punct_kind(&pstate, C_PUNCT_LEFT_BRACE, &tok));
//     dbgp(c_token, tok, .data = &text);
...
//     auto pstate = (ParserState) {
//         .tokens = darr_slice_full(tokens),
//         .ast_alloc = g_ctx.global_alloc,
//     };

//     while (true) {
//         tok = slice_get_T(C_Token, &pstate.tokens, pstate.cur);
//         dbgp(c_token, tok, .data = &text);
//         if (tok->kind == C_TOKEN_KIND_EOF) {
//             break;
//         }
//         parser_advance(&pstate);
//     }
...
//     println_pref(gen_dbg_fmt, (C_Ast_TypeStruct *)decl->ty);
...
test_expr() {
    ...
    // cr_log_warn("%d", g_y);
        ...
        WITH_FILE(S("sandbox/expr_out.dot"), "w", file, {
            ...
            ASSERT_OK(string_formatter_write_fmt(&fmt, S("digraph G {\n")));
            string_formatter_pad_inc(&fmt);

            usize_t index = 0;
            ASSERT_OK(c_ast_expr_compile_graphvis_fmt(expr, &fmt, &index, index));
            // ASSERT_OK(string_formatter_write(&fmt, S("\n")));
        ...
        system("dot -Tpng sandbox/expr_out.dot -o build/expr_out.png");
